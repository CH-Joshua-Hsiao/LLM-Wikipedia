...
def lint_hygiene():
    ...
    if os.path.exists(config.PAGES_DIR):
        for file in os.listdir(config.PAGES_DIR):
            if file.endswith(".md"):
                path = os.path.join(config.PAGES_DIR, file)
    ...
    for mod_file in modified_files:
        path = os.path.join(config.PAGES_DIR, mod_file)
        with open(path, "r", encoding="utf-8") as f:
            content = f.read()
            neighbors = link_pattern.findall(content)
            for neighbor in neighbors:
                if neighbor.startswith(config.PAGES_DIR + "/"):
                    neighbor = neighbor[len(config.PAGES_DIR)+1:]
                if neighbor in os.listdir(config.PAGES_DIR):
...
"""
            ...
            if "YES" in ans:
                ...
                path_a = os.path.join(config.PAGES_DIR, file_a)
                path_b = os.path.join(config.PAGES_DIR, file_b)
                ...
                with conn.cursor() as cur:
                    ...
                    for src in sources:
                        src_path = os.path.join(config.PAGES_DIR, src)
                    ...
                    if os.path.exists("index.md"):
                        ...
                        with open("index.md", "w", encoding="utf-8") as f:
                            for line in idx_lines:
                                if f"({config.PAGES_DIR}/{file_b})" not in line:
                                    f.write(line)
                                    
                # Archive B file locally
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                shutil.move(path_b, os.path.join(config.ARCHIVE_DIR, f"{file_b.replace('.md', '')}_merged_{timestamp}.md"))
